...
####
# This file minimizes distance between brain regions only
####
...
for i in range(n):
    for j in range(n):
        ...
        constr.append(dists[i][j] >= Rij[i][j])
        #print("%i, %i: dist= >= %f" % (i, j, r[i] + r[j]))

#################################################################################
# Problem solver - takes a long time!
prob = cvx.Problem(cvx.Minimize(cvx.sum(cvx.bmat(dists - Rij))), constr)  # Minimize the value (dists) * (intensities)... Should be the same minimization as intensities/dist?
...
load = False  # Set to false if you want to save a new run (i.e. you uncommented above)
if load:
    with open('min_dist.file', 'rb') as f:
        c, dists = pickle.load(f)
else:
    with open('min_dist.file', 'wb') as f:
