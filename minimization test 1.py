...
def solution(xij):
	return np.dot(Iij.flatten(),(-1./xij).flatten()) / 100000  # Intensity / Distance
	
# Minimum distance based on radii + distance guess must be >= 0
eps = 1e-12
def constraint(xij):
	#if min(-Rij.flatten() + xij.flatten()) < 0:
		#print(-Rij.flatten() + xij.flatten())
		#input()
	return (-Rij.flatten() + xij.flatten())

sol = minimize(solution,rij.flatten() + 0.8,method='SLSQP',bounds=Bounds(0.8,100000, keep_feasible=True),constraints=({'type':'ineq','fun':constraint}),options={'disp':True, 'maxiter': 1000})
...
fig.savefig('./minimization troubleshooting/output 100000 bound.png')	
plt.show()
