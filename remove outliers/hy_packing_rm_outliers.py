...
old_rij = rij
rm_nodes = [38, 72, 347]  # Remove these outliers
for id in ids:
    if id in rm_nodes:
        i = ids.index(id)
        del rad[i]
        del names[i]
        del ids[i]
        rij = np.delete(rij, i, axis=0)
        rij = np.delete(rij, i, axis=1)
        Rij = np.delete(Rij, i, axis=0)
        Rij = np.delete(Rij, i, axis=1)
        Iij = np.delete(Iij, i, axis=0)
        Iij = np.delete(Iij, i, axis=1)
n = n - len(rm_nodes)
...
adj_real_dists = pd.read_csv('adjusted HY distances outliers removed.csv', header=None).to_numpy()
...
true_coords = pd.read_csv('true HY coordinates outliers removed.csv', header=None).to_numpy()
true_max_x = 0
true_max_y = 0
true_max_z = 0
true_min_x = np.inf
...
adj_coords = pd.read_csv('adjusted HY coordinates outliers removed.csv', header=None).to_numpy()
adj_max_x = 0
adj_max_y = 0
adj_max_z = 0
adj_min_x = np.inf
