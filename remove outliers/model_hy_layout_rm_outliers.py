...
# centering
real_coords = real_coords + [1, -.5, -4.5]
...
for id in ids:
    if id in rm_nodes:
        ...
        del names[i]
        del ids[i]
        real_coords = np.delete(real_coords, i, axis=0)
n = n - len(rm_nodes)

# Save coords to file
pd.DataFrame(real_coords).to_csv('true HY coordinates outliers removed.csv', index=False, header=False)
# Plot original HY
...
pd.DataFrame(nudged_coords).to_csv('adjusted HY coordinates outliers removed.csv', index=False, header=False)
pd.DataFrame(new_dists).to_csv('adjusted HY distances outliers removed.csv', index=False, header=False)